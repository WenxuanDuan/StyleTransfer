...
def resize_to_match_input(input_path, output_path):
    ...
    try:
        ...
        with Image.open(output_path) as output_img:
            resized_img = output_img.resize((input_width, input_height), Image.LANCZOS)
            resized_img.save(output_path)

        print(f"Resized output image to match input size: {input_width}x{input_height}")
    except Exception as e:
        print(f"Error resizing output image: {e}")

def train(args):
    # 默认设置为 CPU 类型
    use_cuda = False
    dtype = torch.FloatTensor  # 默认使用 CPU 的浮点张量
    ...
    if (args.gpu != None):
        ...
        print
        "Current device: %d" % torch.cuda.current_device()
...
def style_transfer(args):
    ...
    original_size = content.size  # 保留原始尺寸
    content = img_transform(content).unsqueeze(0)
    ...
    # 风格迁移
    with torch.no_grad():
        stylized = style_model(content).cpu().squeeze(0)
        utils.save_image(args.output, stylized)

    # 调整输出图片大小
    resize_to_match_input(args.source, args.output)
